...
export function Popout({ kind, params }: { kind: "editor" | "image"; params: URLSearchParams }) {
  ...
  // close() 只发起关闭;「先 hide 再销毁」那套绕开 WebKit DisplayLink 竞态的动作在 Rust 侧
  // (src-tauri/src/main.rs 的 on_window_event),这里不再拦。
  //
  // 曾经是在这儿用 onCloseRequested + preventDefault 做两步关闭,结果踩到:Tauri v2 只要注册了这个监听,
  // Rust 侧就不自己关窗了,改成等页面调 destroy()。浮窗 webview 一卡死,红点 / Esc / ⌘W 全失效 ——
  // 窗口彻底关不掉,而兜底代码正躺在死掉的那个进程里。移到 Rust 后与页面死活无关。
  const close = () => {
